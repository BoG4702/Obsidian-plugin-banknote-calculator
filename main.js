...
__export(main_exports, {
  ...
  default: () => CashSavingsPlugin
});
module.exports = __toCommonJS(main_exports);
var import_obsidian6 = require("obsidian");

// src/core/types.ts
var DEFAULT_CASH_SAVINGS_SETTINGS = {
  walletPath: "Finance/Wallet.md",
  logsFolder: "Finance/Logs",
  currency: "RUB"
};
function ensureCashSavingsSettings(input) {
  const source = typeof input === "object" && input !== null ? input : {};
  const walletPath = typeof source.walletPath === "string" && source.walletPath.trim().length > 0 ? source.walletPath.trim() : DEFAULT_CASH_SAVINGS_SETTINGS.walletPath;
  const logsFolder = typeof source.logsFolder === "string" && source.logsFolder.trim().length > 0 ? source.logsFolder.trim() : DEFAULT_CASH_SAVINGS_SETTINGS.logsFolder;
  const currency = typeof source.currency === "string" && source.currency.trim().length > 0 ? source.currency.trim().toUpperCase() : DEFAULT_CASH_SAVINGS_SETTINGS.currency;
  return {
    walletPath,
    logsFolder,
    currency
...
function cloneState(state) {
  return {
    ...
    plan: { ...state.plan },
    goal: state.goal ? { ...state.goal } : void 0
  };
}
function ensureOperationPayload(op) {
  var _a;
  return {
    countsDelta: (_a = op.countsDelta) != null ? _a : op.values,
    countsAbsolute: op.countsAbsolute
  };
}
function calcBreakdown(state) {
  const breakdown = {
    banknotes: {},
    coins: {}
  };
...
function diffCounts(prevCounts, nextCounts) {
  const delta = {
    banknotes: {},
    coins: {}
  };
...
var OperationService = class {
  ...
  commitOperation(op) {
    return this.enqueue(async () => {
      ...
      const wallet = await this.walletRepo.updateWallet((fm) => {
        fm.counts = {
      ...
      const entry = {
        ...
        comment: (_b = (_a = op.comment) == null ? void 0 : _a.trim()) != null ? _b : ""
      };
...
var PlanService = class {
  calcProjection(totalNow, monthly, months) {
  ...
  calcRequiredToDeadline(totalNow, target, deadlineISO) {
    const safeTotal = Number.isFinite(totalNow) ? totalNow : 0;
    ...
    const daysLeftRaw = Math.ceil((deadlineMs - nowMs) / (1e3 * 60 * 60 * 24));
    if (daysLeftRaw <= 0) {
      return {
        remaining,
...
var DEFAULT_GOAL_DEADLINE = "2026-12-31";
var DEFAULT_BANKNOTES = [5e3, 2e3, 1e3, 500, 200, 100, 50, 10, 5];
var DEFAULT_COINS = [10, 5, 2, 1];
var positiveIntSchema = external_exports.coerce.number().int().positive();
var nonNegativeIntSchema = external_exports.coerce.number().int().min(0);
var nonNegativeNumberSchema = external_exports.coerce.number().min(0);
var anyIntSchema = external_exports.coerce.number().int();
var countMapSchema = external_exports.record(external_exports.string(), nonNegativeIntSchema);
var deltaMapSchema = external_exports.record(external_exports.string(), anyIntSchema);
var denomSetSchema = external_exports.object({
  banknotes: external_exports.array(positiveIntSchema).min(1),
  coins: external_exports.array(positiveIntSchema).min(1)
});
var walletPlanSchema = external_exports.object({
  monthly: nonNegativeNumberSchema,
  months: nonNegativeIntSchema
});
var walletGoalSchema = external_exports.object({
  target: nonNegativeNumberSchema,
  deadline: external_exports.string().trim().min(1)
});
var walletFrontmatterSchema = external_exports.object({
  type: external_exports.literal("cash_wallet"),
  schema_version: external_exports.literal(1),
  currency: external_exports.string().trim().min(1),
...
var logEntryFrontmatterSchema = external_exports.object({
  id: external_exports.string().trim().min(1).optional(),
  type: external_exports.literal("cash_log"),
  schema_version: external_exports.literal(1),
  wallet_path: external_exports.string().trim().min(1),
  ts: external_exports.string().refine((value) => !Number.isNaN(Date.parse(value)), {
    message: "Expected a valid ISO datetime string"
  }),
  kind: external_exports.enum(["deposit", "withdraw", "set"]),
...
function formatSchemaError(entity, error) {
...
function normalizeDenomList(values, fallback) {
...
function normalizeCountsByDenoms(denoms, source) {
  const counts = {};
...
function createZeroCounts(denoms) {
  return {
...
function normalizeWallet(state) {
  ...
  return {
    ...
    counts,
    plan: {
      monthly: toNonNegativeNumber((_a = state.plan) == null ? void 0 : _a.monthly),
      months: toNonNegativeInt((_b = state.plan) == null ? void 0 : _b.months)
    },
    goal
  };
}
function parseWalletFrontmatter(fm) {
...
function prepareWalletFrontmatterInput(fm) {
  if (!isRecord(fm)) {
    return fm;
  }
  const prepared = { ...fm };
  const rawVersion = prepared.schema_version;
  if (rawVersion === void 0 || rawVersion === null || rawVersion === "") {
    prepared.schema_version = 1;
    return prepared;
  }
  const numericVersion = typeof rawVersion === "number" ? rawVersion : typeof rawVersion === "string" ? Number(rawVersion) : NaN;
...
function parseLogFrontmatter(fm) {
  var _a;
  ...
  return {
    id: (_a = parsed.data.id) != null ? _a : fallbackId,
    type: parsed.data.type,
    schema_version: parsed.data.schema_version,
...
function getDefaultWalletFrontmatter(currency = DEFAULT_CURRENCY) {
  const denoms = {
...
function applyWalletToFrontmatter(frontmatter, wallet) {
...
var LedgerRepository = class {
  constructor(app, settings) {
    this.app = app;
    this.fileByLogId = /* @__PURE__ */ new Map();
    const resolved = ensureCashSavingsSettings(settings);
    this.settings = {
      walletPath: (0, import_obsidian.normalizePath)(resolved.walletPath),
      logsFolder: (0, import_obsidian.normalizePath)(resolved.logsFolder),
      currency: resolved.currency
    };
  }
  async ensureLogsFolder() {
  ...
  async appendLog(entry) {
    ...
    const yaml = (0, import_obsidian.stringifyYaml)(normalizedEntry);
    const content = `---
${yaml}---
...
`;
  ...
  async listRecentLogs(limit, walletPath) {
    if (limit <= 0) {
      return [];
    }
    const folderAbstract = this.app.vault.getAbstractFileByPath(this.settings.logsFolder);
    if (!(folderAbstract instanceof import_obsidian.TFolder)) {
      return [];
    }
    const normalizedWalletPath = (0, import_obsidian.normalizePath)(walletPath);
    const entries = [];
    ...
    for (const file of files) {
      ...
      try {
        ...
        if ((0, import_obsidian.normalizePath)(entry.wallet_path) === normalizedWalletPath) {
          entries.push(entry);
  ...
  async nextLogFilePath(ts, kind) {
    const stemBase = this.makeLogStem(ts, kind);
    let suffix = 0;
    while (true) {
      const suffixPart = suffix === 0 ? "" : `-${suffix}`;
      const candidate = (0, import_obsidian.normalizePath)(
  ...
  makeLogStem(ts, kind) {
    ...
    const safeTsRaw = Number.isNaN(parsed.getTime()) ? ts.replace(/[:.]/g, "-") : parsed.toISOString().slice(0, 19).replace(/:/g, "-");
...
var WalletRepository = class {
  constructor(app, settings) {
    ...
    const resolved = ensureCashSavingsSettings(settings);
    this.settings = {
      walletPath: (0, import_obsidian2.normalizePath)(resolved.walletPath),
      logsFolder: (0, import_obsidian2.normalizePath)(resolved.logsFolder),
      currency: resolved.currency
    };
  }
  getWalletFile() {
  ...
  async ensureWalletExists() {
    ...
    const frontmatterText = (0, import_obsidian2.stringifyYaml)(defaultWallet);
...
`;
  ...
  async readFrontmatterFromCache(file) {
  ...
  async readWalletFrontmatter(file) {
...
var DASHBOARD_VIEW_TYPE = "cash-savings-dashboard";
var DashboardView = class extends import_obsidian3.ItemView {
  ...
  renderWalletError(container, message) {
  ...
  async handleResetWallet() {
    ...
    try {
      ...
      new import_obsidian3.Notice("Wallet YAML reset to defaults.");
      await this.refresh();
    } catch (error) {
  ...
  createActionButton(container, label, kind) {
    const button = container.createEl("button", { text: label });
    button.onclick = () => {
      void this.plugin.openOperationModal(kind);
    };
  }
  renderCountsTable(container, wallet) {
  ...
  renderGroupRows(tbody, wallet, group, title) {
    var _a;
    ...
    for (const denom of wallet.denoms[group]) {
      const key = String(denom);
      const count = (_a = wallet.counts[group][key]) != null ? _a : 0;
  ...
  renderPlanSection(container, wallet, total) {
    var _a, _b, _c, _d, _e, _f, _g, _h;
    const block = container.createDiv({ cls: "cash-savings-plan" });
    block.createEl("h3", { text: "Plan" });
    const monthlyValue = (_b = (_a = wallet.plan) == null ? void 0 : _a.monthly) != null ? _b : 0;
    const monthsValue = (_d = (_c = wallet.plan) == null ? void 0 : _c.months) != null ? _d : 0;
    ...
    goalTargetInput.value = String((_f = (_e = wallet.goal) == null ? void 0 : _e.target) != null ? _f : 0);
    ...
    goalDeadlineInput.value = (_h = (_g = wallet.goal) == null ? void 0 : _g.deadline) != null ? _h : "";
  ...
  async handleSavePlan(monthlyRaw, monthsRaw) {
    const monthly = Number(monthlyRaw);
    const months = Number(monthsRaw);
    if (!Number.isFinite(monthly) || monthly < 0) {
      new import_obsidian3.Notice("Monthly must be a non-negative number.");
      return;
    }
    if (!Number.isInteger(months) || months < 0) {
      new import_obsidian3.Notice("Months must be a non-negative integer.");
      return;
    }
    await this.plugin.walletRepository.updateWallet((fm) => {
    ...
    new import_obsidian3.Notice("Plan saved.");
    await this.refresh();
  }
  async handleSaveGoal(targetRaw, deadlineRaw) {
    const target = Number(targetRaw);
    const deadline = deadlineRaw.trim();
    if (!Number.isFinite(target) || target < 0) {
      new import_obsidian3.Notice("Target must be a non-negative number.");
      return;
    }
    if (target > 0 && deadline.length === 0) {
      new import_obsidian3.Notice("Deadline is required when target is greater than 0.");
      return;
    }
    if (deadline.length > 0 && Number.isNaN(Date.parse(deadline))) {
      new import_obsidian3.Notice("Deadline must be a valid date.");
      return;
    }
    await this.plugin.walletRepository.updateWallet((fm) => {
      if (target <= 0 && deadline.length === 0) {
    ...
    new import_obsidian3.Notice("Goal saved.");
    await this.refresh();
  }
  async renderRecentLogs(container, currency) {
  ...
  buildDeltaSummary(entry) {
    const parts = [];
    for (const group of ["banknotes", "coins"]) {
...
var OperationModal = class extends import_obsidian4.Modal {
  ...
  onOpen() {
    ...
    contentEl.createEl("p", {
      text: this.options.kind === "set" ? "Enter absolute counts for each denomination." : "Enter delta counts for each denomination (default 0)."
    });
  ...
  renderGroupTable(container, group, title) {
    var _a;
    ...
    for (const denom of this.options.walletState.denoms[group]) {
      const key = String(denom);
      const current = (_a = this.options.walletState.counts[group][key]) != null ? _a : 0;
      const row = tbody.createEl("tr");
      row.createEl("td", {
        text: String(denom),
  ...
  buildCountsFromInputs() {
    ...
    for (const group of ["banknotes", "coins"]) {
      for (const denom of this.options.walletState.denoms[group]) {
        ...
        const raw = Number((_a = input == null ? void 0 : input.value) != null ? _a : "0");
  ...
  async handleSave() {
    ...
    const operation = this.options.kind === "set" ? {
      kind: "set",
    ...
    try {
      ...
      await ((_b = (_a = this.options).onCommitted) == null ? void 0 : _b.call(_a));
      this.close();
    } catch (error) {
...
var CashSavingsSettingTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian5.Setting(containerEl).setName("walletPath").setDesc("Path to wallet note").addText(
      (text) => text.setPlaceholder("Finance/Wallet.md").setValue(this.plugin.settings.walletPath).onChange(async (value) => {
        const nextSettings = {
          ...this.plugin.settings,
          walletPath: (0, import_obsidian5.normalizePath)(value.trim() || "Finance/Wallet.md")
        };
        await this.plugin.updateSettings(nextSettings);
      })
    );
    new import_obsidian5.Setting(containerEl).setName("logsFolder").setDesc("Folder for operation logs").addText(
      (text) => text.setPlaceholder("Finance/Logs").setValue(this.plugin.settings.logsFolder).onChange(async (value) => {
        const nextSettings = {
          ...this.plugin.settings,
          logsFolder: (0, import_obsidian5.normalizePath)(value.trim() || "Finance/Logs")
        };
        await this.plugin.updateSettings(nextSettings);
      })
    );
    new import_obsidian5.Setting(containerEl).setName("currency").setDesc("Display currency code").addText(
      (text) => text.setPlaceholder("RUB").setValue(this.plugin.settings.currency).onChange(async (value) => {
        const nextSettings = {
          ...this.plugin.settings,
...
// src/main.ts
var DEFAULT_SETTINGS = DEFAULT_CASH_SAVINGS_SETTINGS;
var CashSavingsPlugin = class extends import_obsidian6.Plugin {
  constructor() {
    super(...arguments);
  ...
  async openOperationModal(kind) {
    try {
      ...
      const modal = new OperationModal(this.app, {
        ...
        onCommitted: async () => {
          await this.refreshDashboardViews();
          new import_obsidian6.Notice("Operation committed.");
        }
      });
      modal.open();
    } catch (error) {
  ...
  normalizeSettings(raw) {
    ...
    const walletPath = source.walletPath.trim().length > 0 ? (0, import_obsidian6.normalizePath)(source.walletPath.trim()) : DEFAULT_SETTINGS.walletPath;
    const logsFolder = source.logsFolder.trim().length > 0 ? (0, import_obsidian6.normalizePath)(source.logsFolder.trim()) : DEFAULT_SETTINGS.logsFolder;
    const currency = source.currency.trim().length > 0 ? source.currency.trim().toUpperCase() : DEFAULT_SETTINGS.currency;
    return {
      walletPath,
      logsFolder,
      currency
